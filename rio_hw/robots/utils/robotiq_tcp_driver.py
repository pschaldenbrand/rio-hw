...
class RobotiqTcpDriver:
    ...
    def connect(self, settle_delay: float = 0.5):
        """Establish TCP connection to the robot. ``settle_delay`` is skipped (0) for quick reconnects."""
        self.close_conn()
    ...
    def _peer_closed_error(self, where: str) -> ConnectionError:
        return ConnectionError(
            f"Robotiq TCP peer closed the connection ({where}). "
            f"Verify the UR socket server is running on {self.robot_ip}:{self.tcp_port}, "
            "the teaching program matches Modbus RTU-over-TCP for the gripper, "
            f"and member_id=0x{self.member_id:02X} matches the gripper slave address."
        )

    def _recv_checked(self, nbytes: int, where: str) -> bytes:
        data = self.sock.recv(nbytes)
        if data == b"":
            self._invalidate_socket()
            raise self._peer_closed_error(where)
        return data

    def _send_frame(self, frame: bytes) -> None:
        if self.sock is None:
            raise self._peer_closed_error("send with no connection")
        try:
            self.sock.sendall(frame)
        except (BrokenPipeError, ConnectionResetError) as e:
            self._invalidate_socket()
            raise self._peer_closed_error(f"send failed: {e}") from e

    def _drain_incoming_reconnect_on_eof(self) -> None:
        """Consume stale RX. If the peer closed (UR often does after a write), open a new TCP session."""
    ...
    def _write_output_registers(self):
        ...
        # UR Modbus socket bridges often close the TCP session after a write. ``update()``
        # rate-limits ``_drain_write_responses()``, so the next ``moveG`` can otherwise send on a
        # half-closed socket and raise BrokenPipeError. Always drain / reconnect-on-EOF before FC16.
        for attempt in range(2):
            if self.sock is None:
                ...
                old_timeout = self.sock.gettimeout()
                try:
                    self._drain_incoming_reconnect_on_eof()
                finally:
                    if self.sock is not None:
                        self.sock.settimeout(old_timeout)
            try:
    ...
    def _read_input_registers(self):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                try:
                    self._send_frame(frame)
                    resp = self._recv_checked(256, "read input registers response")
    ...
    def state(self, refresh=True) -> dict:
        ...
        Args:
            refresh: If True, may query the gripper, but no faster than ``read_interval`` so UR
                socket bridges are not flooded with back-to-back FC03 after ``moveG``/``update``.
        """
        if refresh:
            self._poll_input_registers_if_due()
    ...
    def reset(self):
        ...
        try:
            ...
            # UR Modbus socket bridges often send FIN right after a write with no Modbus reply on
            # the TCP stream; recv then returns b"". Invalidate and let activate() open a new TCP
            # session instead of failing startup.
            if resp == b"":
                self._invalidate_socket()
        except TimeoutError:
            pass
    ...
    def activate(self, timeout=10.0, debug=True):
        ...
        # Many UR socket programs close the TCP session after servicing one Modbus write; the next
        # send then fails with EPIPE while the local socket still looks connected. Open a new
    ...
    def open(self, wait=False, timeout=10.0):
        result = self.moveG(1.0, wait=wait, timeout=timeout)
        self.binary_state = GripperState.OPENED
        return result

    def close(self, wait=False, timeout=10.0):
        result = self.moveG(0.0, wait=wait, timeout=timeout)
        self.binary_state = GripperState.CLOSED
        return result

    def grip_width(self, position: float, wait=False, timeout=10.0):
        return self.moveG(position, wait=wait, timeout=timeout)
